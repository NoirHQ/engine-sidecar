...
impl FromStr for IdentifierWrapper {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        // TODO: Change move-core-types dependency from crates to aptos-core repository
        // Ok(IdentifierWrapper(Identifier::from(s)?))
        Ok(IdentifierWrapper(Identifier::from_utf8(
            s.as_bytes().to_vec(),
        )?))
    }
}
